...
class AdvanceTreeBuilder:
    ...
    def _build_tree_dfs(self, node):
        children = self._get_children_node(node)
    ...
    def _get_children_of_player_node(self, node):
        children = []
        ...
        fold_node.street_player_action_count = node.street_player_action_count + 1
        ...
        if node.street_player_action_count == 0:
            ccall_node = Node(street, board, op, node.bets, NodeTypes.CHECK)
            ccall_node.street_player_action_count = node.street_player_action_count + 1
        ...
        # TRANSITION CALL' street must be in [0, 1, 2], and opponent bet must < stack(20000)
            ...
            terminal_call_node.street_player_action_count = node.street_player_action_count + 1
    ...
    def _get_children_of_chance_node(self, node):
        children = []
        street, board = node.street, node.board
        cp, op = node.current_player, 1 - node.current_player
        ...
        next_boards = self.get_boards(next_street, node.board)  # get all possible future boards
    ...
    def _get_children_node(self, node):
        ...
        if node.terminal:
            return []
        elif cp == Players.CHANCE:
            return self._get_children_of_chance_node(node)
        else:
            assert cp == Players.P0 or cp == Players.P1
            return self._get_children_of_player_node(node)
